...
All of the contract code written for this thesis is written in Solidity and takes advantage of many of the aspects of the language, such as inheritance and modifiers. The following is a collection of relevant features or issues associated with the Solidity language which are needed to understand the code related to this thesis entirely.

### State Variables

State variables are variables whose values are permanently stored with the contract, i.e. the state variables are located in the storage. The state variables are part of the state of the contract and transactions---which have to pay gas---can modify the state of the contract by executing code which modifies those state variables.
...
\lstinputlisting[caption={[OpenZepplin's implementation of the \texttt{onlyOwner} modifier]OpenZepplin's implementation of the \texttt{onlyOwner} modifier which restricts the access to the owner of the contract.},label=lst:OZOnlyOwner,language=Solidity]{lst/onlyOwner.sol}
\end{minipage}
...
Events are an interface in Solidity to interact with the \gls{evm} logging facilities. The central aspect to remember with respect to events is that they are emitted by a contract, but contracts are not able to listen for events. Events are intended for \glspl{dapp} which listen to specific events and for example, can trigger actions or updates on their \gls{ui}.

### View Functions

View functions in Solidity are defined as functions which do not modify the state. As defined in the Solidity documentation \citepalias{soldoc}, modifying the state implies one of:
...
Note that the attributes---including `msg.sender` and `msg.value`---can change for external calls. This means, for example, that if a regular account Alice, calls a contract Carlos, Carlos will see Alice's address as the `msg.sender`. Next, if Carlos makes a call to Carol, another contract, Carol will see the address of Carlos---not Alice---as the `msg.sender`.
